...
2026-09-07 화면 개편 커밋 `8b913c5`를 GitHub `main`에 푸시했습니다. 이번 작업에서 확인한 범위는 로컬 검증과 GitHub 반영이며, 공개 사이트 배포 성공 및 원격 Spring 콘텐츠 반영 여부는 확인하지 않았습니다. 저장소의 GitHub Actions는 검증용이며 배포 워크플로가 아닙니다. 자동 공개 여부는 별도의 Vercel Git 연동과 운영 서버 설정에 달려 있습니다.

공개 운영 시에는 다음 두 작업을 같은 커밋을 기준으로 연결합니다.

- Vercel이 `frontend/`를 빌드하고 배포합니다.
- Spring은 `BLOG_CONTENT_SOURCE=local`이면 해당 커밋의 `frontend/content/`를 서버에 전달하고, `github`이면 설정한 저장소·브랜치에서 직접 조회합니다. 자동 스캔 또는 인증된 수동 재인덱싱으로 반영합니다. [콘텐츠 소스 설정](../backend/README.md#콘텐츠-소스-로컬-vs-github)을 참고합니다.

로컬 콘텐츠 소스에서는 `/admin/reindex` 호출 전에 서버에 최신 파일이 있어야 합니다. GitHub 콘텐츠 소스에서는 재인덱싱 과정에서 GitHub API로 콘텐츠를 읽습니다. 글 삭제·draft 전환도 프론트 재배포와 서버 인덱스 갱신을 함께 확인해야 화면과 검색이 일치합니다.
